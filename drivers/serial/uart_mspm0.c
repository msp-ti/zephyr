...
static int uart_mspm0_irq_tx_complete(const struct device *dev)
{
	...
	return (DL_UART_Main_isBusy(config->regs)) ? 0 : 1;
}
...
static int uart_mspm0_pm_action(const struct device *dev,
			       enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		while(DL_UART_Main_isBusy(config->regs));
		return 0;
...
static int uart_mspm0_init(const struct device *dev)
{
    ...
    DL_UART_Main_setTXFIFOThreshold(config->regs, DL_UART_TX_FIFO_LEVEL_1_4_EMPTY);
	...
	return pm_device_driver_init(dev, uart_mspm0_pm_action);
}
